...
describe("buildTimeline", () => {
  it("メール・メモ・監査を時系列昇順で1本に統合する", () => {
    const items = buildTimeline({
      messages: [
        { id: "m1", sentAt: new Date("2026-06-26T10:00:00Z"), direction: "INBOUND", fromAddr: "c@x.com", subject: "件名", bodyText: "本文", bodyHtml: null, attachments: [] },
        { id: "m2", sentAt: new Date("2026-06-26T12:00:00Z"), direction: "OUTBOUND", fromAddr: "s@x.com", subject: "Re: 件名", bodyText: "返信", bodyHtml: null, attachments: [] },
      ],
      notes: [
  ...
  it("各種目の中身を正しく写像する", () => {
    const items = buildTimeline({
      messages: [{ id: "m1", sentAt: new Date("2026-06-26T10:00:00Z"), direction: "INBOUND", fromAddr: "c@x.com", subject: "件名", bodyText: null, bodyHtml: "<p>x</p>", attachments: [{ id: "at1", filename: "a.pdf", contentType: "application/pdf", size: 10, inline: false }] }],
    ...
    expect(items[0]).toEqual({ kind: "message", id: "m1", at: new Date("2026-06-26T10:00:00Z"), direction: "INBOUND", fromAddr: "c@x.com", subject: "件名", bodyText: null, bodyHtml: "<p>x</p>", attachments: [{ id: "at1", filename: "a.pdf", contentType: "application/pdf", size: 10, inline: false }] });
